...
int	main(void)
{
	...
	m =	new_matrix(table, 4);
	printf("Matrix:\n");
	print_matrix(m);
	printf("Inverse matrix:\n");
	print_matrix(inverse_matrix(m));
	printf("Multiply matrix with the inverse:\n");
	print_matrix(multiply_matrices(m, inverse_matrix(m)));
	printf("\n");
	...
	print_matrix(inverse_matrix(transpose_matrix(m)));
	printf("The transpose of the inverse of a matrix:\n");
	print_matrix(transpose_matrix(inverse_matrix(m)));
	printf("\n");
	...
	printf("%f %f %f %f\n", t.x, t.y, t.z, t.w);
	m = get_identity_matrix();
	t = multiply_matrix_by_tuple(m, t);
