...
void	create_scene(t_world *world)
{
	...
	world->objs_count = 5;
	new_sphere(&c);
	c.material.color = new_color(1.5, 0.5, 0.5);
	...
	c.cast_shadow = false;
	new_sphere(&c1);
	c1.material.color = new_color(0.5, 1.5, 0.5);
	c1.material.ambient = new_color(1, 1, 1);
...
void	create_aspotlights(t_world *world)
{
	...
	asp.usteps = 10;
	asp.vsteps = 10;
	asp.intensity = new_color(1.5, 0.5, 0.5);
	new_point(0, 0, 0, &to);
	...
	asp.usteps = 10;
	asp.vsteps = 10;
	asp.intensity = new_color(0.5, 1.5, 0.5);
	new_point(0, 0, 0, &to);
	new_point(4, 3, 5, &from);
	subtract(&to, &from, &asp.direction);
	asp.center_radian = M_PI_4 / 32; 
...
void	create_spotlights(t_world *world)
{
	...
	sp.intensity = new_color(1.5, 0.5, 0.5);
	...
	new_point(0, 0, 0, &to);
	subtract(&to, &sp.position, &sp.direction);
	sp.intensity = new_color(0.5, 1.5, 0.5);
	sp.center_radian = M_PI_4 / 32; 
	sp.fade_radian = M_PI / 6;
