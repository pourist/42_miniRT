...
void	create_background(t_world *world)
{
	...
	new_checkers_pattern(new_solid_pattern(&a, &tmp[0]),
		new_solid_pattern(&b, &tmp[1]), &floor.material.pattern);
	new_stripe_pattern(
		new_solid_pattern(new_color(0.9, 0.3, 0.3, &c), &tmp[0]),
		new_solid_pattern(new_color(0.6, 0.3, 0.3, &d), &tmp[1]), floor.material.pattern.a);
	set_pattern_transform(floor.material.pattern.a, scaling(0.1, 0.1, 0.1, &multi[0]));
	new_stripe_pattern(
		new_solid_pattern(&a, &tmp[0]), new_solid_pattern(&b, &tmp[1]), floor.material.pattern.b);
	...
	new_blended_pattern(
		new_solid_pattern(&a, &tmp[0]), new_solid_pattern(&b, &tmp[1]), &front_wall.material.pattern);
	new_stripe_pattern(
		new_solid_pattern(new_color(0, 0.5, 0.1, &c), &tmp[0]),
		new_solid_pattern(new_color(0.8, 0.8, 0.8, &d), &tmp[1]), front_wall.material.pattern.a);
	new_stripe_pattern(
		new_solid_pattern(new_color(0, 0.5, 0.1, &c), &tmp[0]),
		new_solid_pattern(new_color(0.8, 0.8, 0.8, &d), &tmp[1]), front_wall.material.pattern.b);
	...
	new_checkers_pattern(new_solid_pattern(&a, &tmp[0]),
		new_solid_pattern(&b, &tmp[1]), &right_wall.material.pattern);
	...
	new_plane(&left_wall);
	new_checkers_pattern(new_solid_pattern(&a, &tmp[0]),
		new_solid_pattern(&b, &tmp[1]), &left_wall.material.pattern);
...
void	create_spheres(t_world *world)
{
	...
	new_uv_checkers_pattern(new_solid_pattern(&a, &tmp[0]),
		new_solid_pattern(&middle.material.color, &tmp[1]), &middle.material.pattern);
	...
	new_ring_pattern(new_solid_pattern(&a, &tmp[0]),
		new_solid_pattern(&right.material.color, &tmp[1]), &right.material.pattern);
	...
	new_full_gradient_pattern(new_solid_pattern(&a, &tmp[0]),
		new_solid_pattern(&left.material.color, &tmp[1]), &left.material.pattern);
