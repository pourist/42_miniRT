...
void	create_scene(t_world *world)
{
	...
	new_checkers_pattern(
		new_solid_pattern(new_color(0.1, 0.1, 0.1, &c)),
	...
	new_cube(&ceiling);
	new_color(0.9, 0.9, 0.95, &ceiling.material.color);
	multiply_matrices(translation(0, 5, 0, &m[0]), scaling(10, 0.01, 10, &m[1]), &m[0]);
	set_transform(&ceiling, &m[0]);
	new_cube(&backwall);
	backwall.material.pattern = new_solid_pattern(new_color(0.6, 0.1, 0.05, &c));
	...
	new_cube(&table_top);
	new_color(0.6, 0.3, 0.1, &table_top.material.color);
	multiply_matrices(translation(0, 1, 0, &m[0]), scaling(2, 0.01, 1, &m[1]), &m[0]);
	set_transform(&table_top, &m[0]);
	new_cube(&leg1);
	new_color(0.6, 0.3, 0.1, &leg1.material.color);
	multiply_matrices(translation(-1.7, 0, -0.9, &m[0]), scaling(0.1, 1, 0.1, &m[1]), &m[0]);
	set_transform(&leg1, &m[0]);
	new_cube(&leg2);
	new_color(0.6, 0.3, 0.1, &leg2.material.color);
	multiply_matrices(translation(1.7, 0, -0.9, &m[0]), scaling(0.1, 1, 0.1, &m[1]), &m[0]);
	set_transform(&leg2, &m[0]);
	new_cube(&leg3);
	new_color(0.6, 0.3, 0.1, &leg3.material.color);
	multiply_matrices(translation(1.7, 0, 0.9, &m[0]), scaling(0.1, 1, 0.1, &m[1]), &m[0]);
	set_transform(&leg3, &m[0]);
	new_cube(&leg4);
	new_color(0.6, 0.3, 0.1, &leg4.material.color);
	multiply_matrices(translation(-1.7, 0, 0.9, &m[0]), scaling(0.1, 1, 0.1, &m[1]), &m[0]);
	set_transform(&leg4, &m[0]);
	new_cube(&mirror);
	new_color(0.1, 0.1, 0.1, &mirror.material.color);
	...
	new_cube(&cube);
	new_color(0.6, 0.7, 0.8, &cube.material.color);
	...
	cone.cone.max = 1.0;
	new_checkers_pattern(
		new_solid_pattern(new_color(0.957, 0.80, 0.604, &c)),
		new_solid_pattern(new_color(0.925, 0.663, 0.333, &c)), &cone.material.pattern);
	...
	new_color(0.957, 0.604, 0.761, &icecream1.material.color);
	...
	new_color(0.604, 0.957, 0.624, &icecream2.material.color);
	...
	ring2.cyl.min = 0.0;
	ring2.cyl.max = 1.0;
	new_color(0.957, 0.604, 0.761, &ring2.material.color);
	...
	cone2.cone.min = -1.0;
	cone2.cone.max = 1.0;
	new_color(0.757, 0.404, 0.561, &cone2.material.color);
...
void	create_ligts(t_world *world)
{
	...
	new_point(-4, 4, -3, &p);
	new_color(0.4, 1.0, 0.7, &c);
	new_light(&p, &c, &world->lights[0]);
	new_point(4, 0.5, -3, &p);
	new_color(0.5, 0.4, 0.7, &c);
